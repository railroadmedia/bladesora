<header id="nav" class="container fluid shadow tw-z-100">
    <div class="{{ isset($stretch) && $stretch ? 'nav-inner' : '' }} container collapsed tw-bg-inherit">
        <div class="tw-flex tw-bg-inherit tw-justify-between">
            <div class="top-navbar-left-content tw-flex tw-grow tw-w-full relative">
                @if (empty($isIframe))
                    <a id="menuButton" tabindex="0" dusk="hamburger-button"
                    ...
                    <a id="colapseButton" tabindex="0" dusk="hamburger-button"
                ...
                <a href="/members" class="flex flex-column logo align-center" @if (!empty($isIframe)) target="_parent" @endif>
                    <img src="{{ $logo }}" style="    @if ($themeColor == 'drumeo')
                    padding:0 0 10px;
                ...
                <div class="flex flex-column align-center">
                    <div class="flex flex-row search-input-container align-v-center hide-sm-down">
                        ...
                        <form method="GET" action="{{ $searchUrl }}">
                            <input id="search-input" name="term" type="text" class="solo text-white"
                                placeholder="What would you like to learn?" autocomplete="off"
                                onfocus="this.placeholder = ''"
                                onblur="this.placeholder = 'What would you like to learn?'" dusk="search-input">
                        </form>
                ...
                <div class="flex header-button live-button flex-column align-v-center noselect ph-1">
                    <a href="{{ $livePageUrl }}" @if (!empty($isIframe)) target="_parent" @endif id="liveIndicator"
                ...
                @if (!empty($links['Notifications']))
                    <div class="flex header-button flex-column noselect">
                        <a href="{{ $links['Notifications']['url'] }}" id="notificationButton" class="square">
                            <div class="pa-1 wrap">
                ...
                @if (!empty($searchUrl) && !$agent->is('IE') && empty($isIframe))
                    <div id="searchColumn" class="flex search-column flex-column noselect align-v-center hide-md-up">
                        <div class="tw-flex search-row tw-justify-end tw-pl-10">
                            <div class="flex flex-column search-button title hover-text-white text-grey-3 align-center pointer search-btn-open"
                                dusk="search-button">
                                <i class="fas fa-search"></i>
                            </div>
                            <div id="searchBox" class="form-group flex grow align-v-center flex-column">
                                <form method="GET" action="{{ $searchUrl }}">
                                    <input id="toggled-search-input" name="term" type="text" class="solo text-white"
                                        placeholder="What would you like to learn?" onfocus="this.placeholder = ''"
                                        onblur="this.placeholder = 'What would you like to learn?'" autocomplete="off"
                                        dusk="search-input">
                                </form>
                            </div>
                            <div
                                class="flex flex-column search-button title hover-text-white text-grey-3 align-center pointer search-btn-close">
            ...
            <div class="top-navbar-right-content tw-flex tw-justify-end">
                @if (isset($showReferralButton) && $showReferralButton)
                    <div class="pa-1 noselect tw-grow-0 tw-bg-inherit referral-btn"
                        style="padding-right: 0; z-index: 10">
                        <a href="{{ $referralPageUrl ?? '/' }}"
                            class="btn tw-w-7 tw-h-7 lg:tw-h-9 md:tw-w-36 tw-rounded-full text-white bg-{{ $themeColor }}"
                            style="font-size: 13px;">
                            <i class="fas fa-gift md:tw-mr-2"></i> <span class="tw-hidden md:tw-inline-block">invite a
                                friend</span>
                        </a>
                    </div>
                @endif
                <div class="flex header-button flex-column noselect relative">
                    <a href="{{ $accountUrl }}" class="square profile-nav-link" dusk="profile-nav-link"
...
@if (empty($isIframe))
                ...
                <div class="member-avatar">
                    <a href="{{ $member['profileUrl'] ?? '#' }}">
                        <img class="bg-grey-2 rounded {{ $themeColor }}"
                            src="{{ imgix($member['avatar'], ['q' => 50, 'blur' => 2, 'w' => 60, 'h' => 60, 'fit' => 'crop']) }}"
                            alt="member avatar">
                    </a>
                </div>
        ...
        <section id="pageLinks" class="flex flex-column">
            ...
            <div class="sub-links flex flex-column mt-3">
                ...
                @if (!empty($drumeoBeatUrl))
                    <a class="flex flex-row align-v-center text-black ph mb-1" href="{{ $drumeoBeatUrl }}"
                        target="_blank">
                ...
                @if (!empty($legacyResourcesUrl))
                    <a class="flex flex-row align-v-center text-black ph mb-1"
                        href="{{ $legacyResourcesUrl }}">Legacy Resources</a>
                @endif

                <a class="flex flex-row align-v-center text-black ph mb-1" href="{{ $supportUrl }}"
                    dusk="page-link-support">Support</a>
                <a class="flex flex-row align-v-center text-black ph mb-1" href="{{ $logoutUrl }}"
                    dusk="page-link-logout">Logout</a>
            </div>
        </section>
    </aside>
    ...
    @if (!empty($levelUpNotification))
        <input id="levelUpData" type="hidden" data-old-rank="{{ $levelUpNotification['tierPrevious'] }}"
